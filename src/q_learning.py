...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        for i in range(0, steps):
          #epsilon
          eps = self._get_epsilon(i/steps)
          if(float(np.random.rand(1)) > eps):
            #saw on campuswire
            #to do random action if everything is 0
            if np.allclose(state_action_values,np.zeros(shape=[num_states, num_action])):
              A = np.random.randint(num_action)
            else:
              A = np.argmax(state_action_values[cur_state])
          else:
            A = np.random.randint(num_action)
        
          obs, R, done, info = env.step(A)

          old_state = state_action_values[cur_state][A]
          max = np.max(state_action_values[obs])

          N[cur_state][A] = N[cur_state][A]+1
          new_state = old_state+ (1 / N[cur_state][A]) * (R + gam * max - old_state)

          state_action_values[cur_state][A] = new_state 
    ...
    def predict(self, env, state_action_values):
        ...
        while not done:
          best_action = np.argmax(state_action_values[cur_state])
